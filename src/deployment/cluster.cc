...
void DeployOne(int nodeID,
  ...
  // Change here: set ssh to connect as root
  char remote_host_with_user[256];
  snprintf(remote_host_with_user, sizeof(remote_host_with_user), "root@%s", node->host.c_str());
  const char* remote_opt2 = remote_host_with_user;
...
void KillRemote(const Configuration& config,
  ...
  for (map<int, Node*>::const_iterator it = config.all_nodes.begin() ;
    ...
    int pid = fork();
    if (pid == 0) {
      // Use root@host for ssh login
      char remote_host_with_user[256];
      snprintf(remote_host_with_user, sizeof(remote_host_with_user), "root@%s", node->host.c_str());
      execlp("ssh", "ssh", remote_host_with_user,
